...
// Asymmetric slew-rate limiter (§5): limit the ramp UP (accelerating,
// |target| > |current|) by accelStep, but allow a faster ramp DOWN
// (decelerating toward zero) by decelStep. Lets a gentle launch coexist
// with a quick stop. Call each control tick.
